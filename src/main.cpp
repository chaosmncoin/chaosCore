...
#if defined(NDEBUG)
#error "Chaos Coin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uhao) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK()) nSubsidy = 200000 * COIN;
    else if (nHeight <= 565345) nSubsidy = 184.94 * COIN;
    else if (nHeight <= 565346) nSubsidy = 83.70 * COIN;
...
{
    RenameThread("chaoscoin-scriptch");
